...
onMounted(() => {
  context = gsap.context(() => {
    ...
    // Project cards: scrub-in with 3D tilt
...
<template>
  <section id="projects" ref="sectionRef" class="relative mx-auto w-full max-w-6xl px-4 py-20 sm:snap-start sm:px-6 md:py-28" style="perspective: 1200px">
    <div ref="headingRef" class="mb-10 flex items-end justify-between gap-4">
      <div>
        <p class="mb-3 text-sm uppercase tracking-[0.18em] text-cyan-300">Selected Work</p>
        <h2 class="text-2xl font-semibold text-white sm:text-3xl md:text-4xl">Bento-Style Projects Grid</h2>
      </div>
    </div>
